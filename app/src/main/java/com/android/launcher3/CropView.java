    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        synchronized (mLock) {
            ...
            if (mRenderer.source != null) {
                ...
                if (edges.top > 0) {
                    adjustment[1] = (float) Math.ceil/* modify by jackson FloatMath */(edges.top / scale);
                } else if (edges.bottom < getHeight()) {
                    adjustment[1] = (edges.bottom - getHeight()) / scale;
                }
                for (int dim = 0; dim <= 1; dim++) {
                    if (coef[dim] > 0) adjustment[dim] = (float) Math.ceil/* modify by jackson FloatMath */(adjustment[dim]);
                }
